function [m_ec, m_nacelle_group] = get_Raymer_Installed_Engine( ...
        m_engine, K_tr, K_ng, K_p, ...
...
m_engine = conv_unit("kg", "lbs", m_engine);
N_lt = conv_unit("m", "ft", N_lt);
N_w = conv_unit("m", "ft", N_w);
S_n = conv_unit("m2", "ft2", S_n);

%% --- Engine + contents weight (per nacelle) ---
% W_ec ≅ 2.331 * W_engine^0.901 * K_p * K_tr
m_ec = 2.331 * (m_engine^0.901) * K_p * K_tr;
...
%% Convert to SI units
m_ec = conv_unit("lbs", "kg", m_ec);
m_nacelle_group = conv_unit("lbs", "kg", m_nacelle_group);
